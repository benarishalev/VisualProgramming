...
void Line::Draw(SDL_Renderer* renderer, std::vector<Node> nodes) {
    this->lengthPercentage = std::min(this->lengthPercentage+0.1f, 100.0f);
    ...
    Point finalPosition = {(float)(nodes[this->start].position.x + cos(angle) * ((distance*lengthPercentage/100) - 30)), (float)(nodes[this->start].position.y + sin(angle) * ((distance*lengthPercentage/100) - 30))};
    ...
    // dont draw the arrow until distance is 20%
    if (this->lengthPercentage < 20) {return;}
    SDL_FRect rect;
